...
# 定义新的模型函数（基于线性叠加原理）
def model(t, k, V):
    D1, D2, D3 = 10, 50, 40  # 各阶段给药剂量
    t1, t2, t3 = 0, 2, 60    # 各阶段开始时间
    
    C1 = D1 * (1 - np.exp(-k*(t-t1))) * np.exp(-k*(t-t1)) * H(t-t1) / V
    C2 = D2 * (1 - np.exp(-k*(t-t2))) * np.exp(-k*(t-t2)) * H(t-t2) / V
    C3 = D3 * (1 - np.exp(-k*(t-t3))) * np.exp(-k*(t-t3)) * H(t-t3) / V
    
    return C1 + C2 + C3

# 使用插值后的数据进行拟合
popt, _ = curve_fit(model, time_interp, conc_interp, p0=[0.1, 22.84])
...
conc_fit = model(time_points, k_fit, V_fit)
...
# 输出拟合参数
print(f"拟合参数：k = {k_fit:.4f}, V = {V_fit:.4f}")
